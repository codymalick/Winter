...
related_(X,Y) :- ancestor(X,Y); ancestor(Y,X); aunt(X,Y); aunt(Y,X); uncle(X,Y); uncle(Y,X); cousin(X,Y); siblingInLaw(X,Y); sibling(X,Y).
related(X,Y) :- related_(X,Z), related_(Z,Y).
...
prog([], S, S).
prog([C|CS], S1, S2) :- cmd(C, S1, SX), prog(CS, SX, S2).
