...
public class BeaverMart {
	...
	// Store pairs that exist already from this record
	List<String> duplicates = new ArrayList<String>();

	// For each value in a record
	for(Iterator<String> i = tokens.iterator(); i.hasNext(); ) {
		...
		for(Iterator<String> j = others.iterator(); j.hasNext(); ) {
			...
			String rev_val = item2 + "," + item;

			if(item != item2 && !duplicates.contains(rev_val)) {
				// Write out
				context.write(new Text(val), one);
				// Add pair to duplicates
				duplicates.add(val);
			}
		}
	}
